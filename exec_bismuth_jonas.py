...
cm = 1/2.54
fig = plt.figure(layout="constrained", dpi = 500, figsize=(8*cm,2))
left, right, sum_up = fig.subfigures(nrows=1, ncols=3)
fig.tight_layout()

axd_01 = left.subplot_mosaic(mosaic, width_ratios = [0.25,1], height_ratios = [0.1,1])
...
ax_x_integrate.set_xticks([0,200,400])
...
ax_detector.text(13, 1350, "Before laser\npulse", multialignment='center')
...
if rotate_detector == True:
    ...
    #ax_y_integrate.text(5200, 1204, "a)")
    ax_y_integrate.plot(y_axis_sums[0][1], y_axis_sums[0][0], marker = 'o', linestyle = "None")
...
axd_02 = right.subplot_mosaic(mosaic, width_ratios = [0.25,1], height_ratios = [0.1,1])
...
ax_detector.tick_params(which='both', top = True, left = True, right = True, bottom = True, direction = "in", width=1)
ax_detector.set_xticks([0,200,400])
ax_detector.text(50, 1350, "After laser\npulse", multialignment='center')
...
if rotate_detector == True:
    ax_x_integrate.plot(x_axis_sums[1][0], x_axis_sums[1][1], marker = 'o', linestyle = "None", color = "orange")
    ax_x_integrate.set_xlim(fast_scan_integrator.ylims)

    #ax_y_integrate.text(5200, 1204, "b)")
    ax_y_integrate.plot(y_axis_sums[1][1], y_axis_sums[1][0], marker = 'o', linestyle = "None", color = "orange")
    ax_y_integrate.invert_xaxis()
    ax_y_integrate.set_ylim(fast_scan_integrator.xlims)
# else:
#     ax_x_integrate.plot(y_axis_sums[1][0], y_axis_sums[1][1], marker = 'o', markersize = 4, linestyle = "None")
...
axd_02 = sum_up.subplot_mosaic(mosaic2, height_ratios = [0.1,1], width_ratios = [0.65,0.35])
ax_y_integrate = axd_02['A panel']
#ax_y_integrate.text(3700, 1204, "c)")
ax_y_integrate.plot(y_axis_sums[0][1], y_axis_sums[0][0], marker = 'o', linestyle = "None")
ax_y_integrate.plot(y_axis_sums[1][1], y_axis_sums[1][0], marker = 'o', linestyle = "None", color = "orange")
ax_y_integrate.tick_params(which='both', top = True, left = True, right = True, bottom = True, direction = "in", width=1)
ax_y_integrate.invert_xaxis()
ax_y_integrate.set_yticklabels("")
ax_y_integrate.set_xticklabels("")
ax_y_integrate.tick_params(direction = "in")
