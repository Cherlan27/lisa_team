...
class water_training():
    def __init__(self, layer, thickness, roughness, sld):
        self.q = np.linspace(0.001, 1, 100)
        self.layer_num = layer
        self.structure = structure_creater(layer, thickness, roughness, sld)
    ...
    def save_loss(self, layer_num: int):
        df = pd.DataFrame()
        name_file = "loss_val" + str(layer_num)
        df["loss"] = self.hist.history["loss"]
        df["val_loss"] = self.hist.history["val_loss"]
        df.to_csv(name_file)
    ...
    def save_fit(self, layer_num: int):
        df = pd.DataFrame()
        name_file = "real_target_val" + str(layer_num)
        df["roughness"] = self.predicted_test_labels["H2O_roughness"]
        df["target_roughness"] = self.test_labels["H2O_roughness"]
        df["sld"] = self.predicted_test_labels["H2O_sld"]
        df["target_sld"] = self.test_labels["H2O_sld"]
        df.to_csv(name_file)
