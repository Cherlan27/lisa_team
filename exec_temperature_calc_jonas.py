...
def fresnel(qz, qc = 0.0216, roughness=0):
    """
...
def error_2(xData, amp, mu, sigma):
    ...
    Array
        y-values for the exponential function            
    """
    return amp*erf((xData - mu)/(sigma*np.sqrt(2))) + 9.2
...
fig, (ax1) = plt.subplots(1, 1, figsize = (8.85*cm, 5.65*cm), dpi = 600, sharex=True)
...
ax1.plot(np.array(time_axis[0:9]), error(np.array(time_axis[0:9]), *params_01), color = "red")
ax1.plot(np.array(time_axis[final_1:final_2]), error_2(np.array(time_axis[final_1:final_2]), *params_02), color = "red")
...
value_qz = np.where(np.round(refnx_xrr1[0],2) == 0.3)
ratio = refnx_xrr1[1][value_qz]/np.mean(concated_params[1][0:4])

fig = plt.figure(dpi = 300)
ax = fig.gca()
ax.errorbar(refnx_xrr1[0], refnx_xrr1[1], refnx_xrr1[2], color = "blue", marker = "o",mfc='none', linestyle = "")
#ax.plot(refnx_xrr1[0], refnx_xrr1[3], color = "blue", marker = "", linestyle = "-")
ax.plot(refnx_xrr1[0], fresnel(refnx_xrr1[0], 0.0216, 2.72)/fresnel(refnx_xrr1[0], 0.0217, 0), color = "blue", marker = "", linestyle = "-")
ax.plot(0.3, np.mean(concated_params[1][0:4])*ratio, marker = "o", markerfacecolor = "green", markeredgecolor = "green")
ax.plot(0.3, np.mean(concated_params[1][8])*ratio, marker = "o", markerfacecolor = "red", markeredgecolor = "red")
...
#ax.errorbar(refnx_xrr1[0], refnx_xrr1[1], refnx_xrr1[2], color = "blue", marker = "o",mfc='none', linestyle = "")
#ax.plot(refnx_xrr1[0], refnx_xrr1[3], color = "blue", marker = "", linestyle = "-")
ax.plot(refnx_xrr1[0], fresnel(refnx_xrr1[0], 0.0216, 2.735)/fresnel(refnx_xrr1[0], 0.0217, 0), color = "green", marker = "", linestyle = "-")
ax.plot(refnx_xrr1[0], fresnel(refnx_xrr1[0], 0.0216, 2.805)/fresnel(refnx_xrr1[0], 0.0217, 0), color = "red", marker = "", linestyle = "-")
ax.plot(0.3, np.mean(concated_params[1][0:4])*ratio, marker = "o", markerfacecolor = "green", markeredgecolor = "green", markersize = 3)
ax.plot(0.3, np.mean(concated_params[1][8])*ratio, marker = "o", markerfacecolor = "red", markeredgecolor = "red", markersize = 3)
plt.xlim([0.28, 0.32])
plt.ylim([0.4, 0.6])
plt.xlabel(r'$q_z$ in $\mathring{A}^{-1}$')
plt.ylabel(r'R/R$_F$')
...
q_z_min = 0.3*10**(10)
delta_b = 20*10**(-6)
bolt_const = 1.380649*10**(-23)

# Temperature calculation
delta_t = (2*np.pi*gam/(bolt_const*np.log(np.sqrt(2)*q_max/(q_z_min*delta_b))))*(2.79*10**(-10))**2 - (2*np.pi*gam/(bolt_const*np.log(np.sqrt(2)*q_max/(q_z_min*delta_b))))*(2.73*10**(-10))**2
print(f"Temperature: {delta_t} K")
delta_t_error = (2*np.pi*gam/(bolt_const*np.log(np.sqrt(2)*q_max/(q_z_min*delta_b))))*2*(2.79*10**(-10))*(0.005*10**(-10)) + (2*np.pi*gam/(bolt_const*np.log(np.sqrt(2)*q_max/(q_z_min*delta_b))))*2*(2.73*10**(-10))*(0.005*10**(-10))
print(f"Temperature error: {delta_t_error} K")


# Double check calculation
rough = np.sqrt((bolt_const)*294/(2*np.pi*gam)*np.log((np.sqrt(2)*q_max)/(q_z_min*delta_b)))*10**(10)
print(rough)