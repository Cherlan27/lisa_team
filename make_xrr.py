...
from eigene.abs_overlap_fit_poly_v2 import Absorber
...
class make_xrr():
    def __init__(self,
                 ...
                 out_data_directory = "/mnt/processed",
        ...
        self.flatfield = "./eigene/Module_2017-004_GaAs_MoFluor_Flatfielddata.tif"
        self.pixel_mask = "./eigene/Module_2017-004_GaAs_mask.tif"
    ...
    def get_scans(self, scan_numbers):
        ...
        for scan_number in scan_numbers:
            ...
            with open(fio_filename, 'r') as fp:
                ...
                for line in lines:
                    ...
                    if line.find("rois") != -1:
                        roi_string = line
            roi_list = roi_string.split("[")[1].replace("]", "").split(",")
            roi_number = 5
            roi_list = (int(roi_list[roi_number*4-4]), int(roi_list[roi_number*4-3]), int(roi_list[roi_number*4-2])-int(roi_list[roi_number*4-4]), int(roi_list[roi_number*4-1]) - int(roi_list[roi_number*4-3]))
            print(roi_list)
            self.roi = roi_list
    ...
    def saving_xrr_data(self, filename):
        ...
        if self.save_results:
            out_filename = file_path + "/" + filename + ".dat"
            df = pandas.DataFrame()
            df["//qz"] = self.qz
    ...
    def plot_xrr(self):
        ...
        plt.show()
        out_plotname = "H:/supex623/supex623/LTP_2023_11/reflectivity/xrr_{0}_{1}.png".format(self.out_experiment, self.scan_numbers[0])
    ...
    def plot_xrr_crit(self):
        ...
        plt.show()
        out_plotname = "H:/supex623/supex623/LTP_2023_11/reflectivity/xrr_{0}_{1}_crit.png".format(self.out_experiment, self.scan_numbers[0])
        plt.savefig(out_plotname, dpi=300)
