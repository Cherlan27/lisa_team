...
class p08_detector_read(object):
    ...
    def pilc_(self):
        detector_file_adc = f"{self._path}/{self._experiment}_{self._scannumber:05}/{self._detector}/{self._experiment}_{self._scannumber:05}_adc_{0:05}.nxs"
        detector_file_cnt = f"{self._path}/{self._experiment}_{self._scannumber:05}/{self._detector}/{self._experiment}_{self._scannumber:05}_cnt_{0:05}.nxs"
        det_file_adc = h5py.File(detector_file_adc, "r")
        det_file_cnt = h5py.File(detector_file_cnt, "r")
        data = dict(ion1 = numpy.array(det_file_adc["/entry/data/value1"]),
            ion2 = numpy.array(det_file_adc["/entry/data/value3"]), 
                 apd = numpy.array(det_file_cnt["/entry/data/value2"]),
